$(function(){
	...
	//初始化 tab1的内容
	// $("#tab1").find('.box').append(listGet(10, 1, 1, list1));
	$("#tab1").append(" <b>Hello world!</b>");

	if(sessionStorage.getItem("list") != null){
	  $('.weui_tab').html(sessionStorage.getItem("list"));
	  console.log('读取缓存');
	}else{
	  console.log('第一次打开');
	}
	...
	$(window).bind('beforeunload',function(){
	  if(sessionStorage.getItem("list") == null){
	    ...
	    sessionStorage.setItem('list', HC);
	    console.log('创建缓存');
	  }else{
	    var HC = $('.weui_tab').html();
	    sessionStorage.setItem('list', HC);
	...
	if($("#page-infinite-navbar")[0]) {
	  ...
	  $("#tab1").infinite().on("infinite", function() {
	    ...
	    setTimeout(function() {
	      //添加分页
	      $(self).find('.box').append(listGet(num, page, 1, list1));
	      p1 += 1;
	      self.loading = false;
	    }, 2000);   //延迟时间2000
	  ...
	  $("#tab2").infinite().on("infinite", function() {
	    ...
	    setTimeout(function() {
	      //添加分页
	      $(self).find('.box').append(listGet(num, page, 2, list2));
	       p2 += 1;
	      self.loading = false;
	    }, 2000);   //延迟时间2000
